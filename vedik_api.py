...
def koordinat_al(sehir):
    geolocator = Nominatim(user_agent="vedik_api_final", timeout=15)
...
def hesapla(tarih, saat, sehir, utc_offset=3):
    ...
    gezegenler["Ke"] = {"ad": "Ketu", "burc": kb["ad"], "derece": kb["derece"], "ev": int(((ketu_ekl - lagna_ekl) % 360) / 30) + 1, "nakshatra": kn["ad"]}
    return {"lagna": lagna, "gezegenler": gezegenler}

def liste_olustur(veri, girdi):
    l, g = veri["lagna"], veri["gezegenler"]
    s = ["="*50, f" VEDIK ANALIZ: {girdi['sehir'].upper()}", "="*50, f" LAGNA: {l['ad']} {l['derece']}"]
...
# --- YOLLAR ---
@app.route("/")
def ana(): return "API Aktif. /saglik kontrol edin."

@app.route("/saglik")
def saglik(): return jsonify({"durum": "aktif", "mesaj": "Sistem Hazir"})
